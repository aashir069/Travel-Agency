...
document.getElementById("reservation-form").addEventListener("submit", function(e) {
    ...
    if (!phonePattern.test(number.value.trim())) {
        showError(number, "Valid phone number required (e.g., 03xx xxxxxxx).");
    }

    // Guests validation
    const guest = document.querySelector("select[name='guest']");
    if (guest.selectedIndex === 0) {
        showError(guest, "Please select number of guests.");
    }
    ...
    if (!date.value) {
        showError(date, "Please select a check-in date.");
    } else if (date.value < today) {
        showError(date, "Check-in date cannot be in the past.");
    }
